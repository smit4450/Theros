---
title: Lagonna-Band Elder
obsidianUIMode: preview
cssclasses: json5e-object
tags:
- ttrpg-cli/compendium/src/5e/tbvxv
- ttrpg-cli/monster/cr/1-2
- ttrpg-cli/monster/size/m
- ttrpg-cli/monster/type/fey
statblock: inline
aliases: ["Lagonna-Band Elder"]
---
# Lagonna-Band Elder
*Source: Theros Bestiary TBVXV*  

<small><blockquote>"The best lessons are not the ones I teach. They are the ones the pupils realize for themselves."</blockquote></small>

![Lagonna-Band Elder](https://img.scryfall.com/cards/art_crop/front/6/8/68b9df54-e78e-4a86-a8b1-b735ec08a812.jpg?1562819312#right)  

```statblock
"name": "Lagonna-Band Elder (TBVXV)"
"size": "Medium"
"type": "fey"
"subtype": "centaur"
"alignment": "Lawful Neutral"
"ac": !!int "12"
"ac_class": "padded"
"hp": !!int "12"
"hit_dice": "2d8 + 4"
"modifier": !!int "1"
"stats":
  - !!int "17"
  - !!int "13"
  - !!int "14"
  - !!int "10"
  - !!int "14"
  - !!int "12"
"speed": "40 ft."
"saves":
  - "wisdom": !!int "4"
"skillsaves":
  - "name": "[[skills#History|History]]"
    "desc": "+2"
  - "name": "[[skills#Insight|Insight]]"
    "desc": "+4"
  - "name": "[[skills#Persuasion|Persuasion]]"
    "desc": "+3"
  - "name": "[[skills#Survival|Survival]]"
    "desc": "+4"
"senses": "passive Perception 10"
"languages": "Sylvan, Common"
"cr": "1/2"
"traits":
  - "desc": "If the centaur moves at least 30 feet straight toward a target and then hits it with a melee attack on the same turn, it can immediately follow that attack with a bonus action, making one attack against the target with its hooves."
    "name": "Charge"
  - "desc": "The centaur counts as one size larger when determining its carrying capacity and the weight it can push or drag. In addition, any climb that requires hands and feet is especially difficult for it because of its equine legs. When it makes such a climb, each foot of movement costs it 4 extra feet instead of the normal 1 extra foot."
    "name": "Equine Build"
  - "desc": "At the start of combat, if the centaur can detect a friendly spell within 30 feet of itself, it gains 3d8 temporary [[hit-points-xphb|Hit Points]] until the end of combat."
    "name": "Motivated by Magic"
"actions":
  - "desc": "Melee Weapon Attack: one target. Hit: 5 (1d4 + 3) bludgeoning damage."
    "name": "Hooves"
"source":
  - "TBVXV"
"image": "Compendium/bestiary/fey/token/lagonna-band-elder-tbvxv.webp"
```
^statblock